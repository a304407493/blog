...
_extend(KEdit, KWidget, {
	init : function(options) {
		...
		//1.
		self.srcElement = K(options.srcElement);//文本框
		//alert("options.srcElement:"+options.srcElement);//object HTMLTextAreaElement
		...
		function ready() {
			...
			//3.处理之后
			//alert("_elementVal(self.srcElement):"+_elementVal(self.srcElement));//空
			if (_IE) {
...
function KEditor(options) {
	...
	//alert("self.srcElement："+self.srcElement);
	//alert(" K(self.srcElement || '<textarea/>'):"+ K(self.srcElement || '<textarea/>'));//[Object Object]
	
	//7.
	//如果没有宽度——设置他为宽度：se.width()
	if (!self.width) {
		self.width = se[0].style.width || se.width();
		//alert("se.width():"+se.width());
		//alert("se[0].style.width："+se[0].style.width);
		//alert("se——self.width："+self.width);
	}
	if (!self.height) {
...
KEditor.prototype = {
	...
	create : function() {
		...
		//9.
		//fullscreenMode——是否充满屏幕
		//_docElement().clientWidth_1351
		//自身的宽度：self.width 141
		var width = fullscreenMode ? _docElement().clientWidth + 'px' : self.width,
			height = fullscreenMode ? _docElement().clientHeight + 'px' : self.height;
		//alert("fullscreenMode:"+fullscreenMode);
		//alert("_docElement().clientWidth:"+_docElement().clientWidth);
		//alert("self.width："+self.width);
		//alert("width:"+width);
		if ((_IE && _V < 8) || _QUIRKS) {
		...
		//layout-1. ——工具栏
		//layout : '<div class="container"><div class="toolbar"></div><div class="edit"></div><div class="statusbar"></div></div>'
		//layout有三个样式——所以container也有三个样式——toolbar、edit、statusbar
		...
		//layout-2. 工具栏赋值给三个变量toolbarDiv、editDiv、statusbar
		//取出这三个样式
		var toolbarDiv = K('.toolbar', container),
			editDiv = K('.edit', container),
			statusbar = self.statusbar = K('.statusbar', container);
		//layout-3 指定样式——self.themeType
		//layout-3. 移除container样式，添加 指定样式ke-container ke-container
		//10.关键点-141是正确——却出现了一个650px;
	...
	resize : function(width, height, updateProp) {
		...
		var self = this;
		//alert("之前：updateProp:"+updateProp);
		updateProp = _undef(updateProp, true);
		//alert("之后：updateProp:"+updateProp);
		if (width) {
			//14.1.此处看不懂
			if (!/%/.test(width)) {
				//alert("!/%/.test(1)："+!/%/.test('1'));
				//alert("!/%/.test(width)："+!/%/.test(width));
				//14.2 函数1.看不懂
				width = _removeUnit(width);//167-141=25
				//alert("_removeUnit(width)："+width);
				//14.3
				width = width < self.minWidth ? self.minWidth : width;
				//alert("self.minWidth:"+self.minWidth);
				//alert("min比较之后width:"+width);//最小值为650
			}
			//15.
			//alert("resize-处理之后-width:"+width);
			//16.编辑器的宽度
